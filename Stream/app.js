...
authorize(startLocalServer);
function authorize(callback) {
  var clientSecret = process.env.CLIENT_SECRET;
  var clientId = process.env.CLIENT_ID;
  var redirectUrl = process.env.REDIRECT_URIS;
...
async function readToken(){
  try {
    const gcreds=await db.getToken()
    oauth2Client.credentials = gcreds;
...
function downloadFile(fileId, access_token, start, end, pipe, onEnd, onStart){
  ...
  var chunkName = fileId + '@' + startChunk
...
function httpDownloadFile(fileId, access_token, start, end, pipe, onEnd, onStart){
  ...
  callback = function(response) {
    ...
    response.on('data', function (chunk) {
      ...
      var chunkName =  fileId + '@' + nextChunk
...
function getFileInfo(fileId, access_token, onData){
  var options = {
    host: 'www.googleapis.com',
    path: '/drive/v3/files/'+fileId+'?alt=json&supportsAllDrives=true&fields=*',
    method: 'GET',
    headers: {
      'Authorization': 'Bearer ' + access_token
    }
...
function addInfo(fileId, fileInfo){
  ...
  info.getVideoLength = new Promise((resolve, reject) => {
    if(!info.videoLength){
      getVideoDurationInSeconds('https://golden-kangaroo-e8a68d.netlify.app' + '/' + fileId).then((duration) => {
